...
public class CategoryFragment extends BaseFragment implements CategoryContract.CategoryView {
    ...
    List<GoodCategory> goodsCategoryList_one = new ArrayList();
    List<GoodCategory> goodsCategoryList_three = new ArrayList();
    List<GoodCategory> goodsCategoryList_two = new ArrayList();
    ...
    public void addGoodsClass(GoodCategory classItem, int m) {
        goodsCategoryList_one=CategoryPresenterImpl.getOneGoodCategoryList();
        goodsCategoryList_two=CategoryPresenterImpl.getTwoGoodCategoryList();
        goodsCategoryList_three=CategoryPresenterImpl.getOneGoodCategoryList();
        AddViewHolder holder = new AddViewHolder(getActivity(), R.layout.category_type_item);
        holder.setText(R.id.tvGoodsClassId, String.valueOf(classItem.getCategoryId())).setText(R.id.tvGoodsClassName, classItem.getCategoryName()).setImageGrey(R.id.ivGoodsClassImage, classItem.getAppImageUrl());
        if (m == 0) {
        ...
        holder.setOnClickListener(R.id.llView, new View.OnClickListener() {
            public void onClick(View view) {
                if (m != CategoryFragment.this.currentItem) {
                    ...
                    CategoryFragment.this.svGoodsClassRoot.smoothScrollTo(0, view.getTop());
    ...
    public void showGoodsClass(String classId) {
    ...
    public void showRightView(final GoodCategory goodsCategory, int position) {
        AddViewHolder holder = new AddViewHolder(getActivity(), R.layout.home_item_goods);
        GridView gvGoodsClass = (GridView) holder.getCustomView().findViewById(R.id.gvGoodsClass);
        if (position == 0) {
            holder.setVisible(R.id.catrgory_line, false);
        }
        switch (position % 10) {
            case 0:
    ...
    private void setCurrentGoodsClass(AddViewHolder holder, String url) {
        holder.setTextColor(R.id.tvGoodsClassName, R.color.colorPrimary).setBackgroundColor(R.id.catrgory_line, R.color.colorPrimary).setDimensionPixelSize(R.id.catrgory_line, R.dimen.tv_line).setImageRed(R.id.ivGoodsClassImage, url);
    }

    private void resetCurrentGoodsClass(AddViewHolder holder, String url) {
        holder.setTextColor(R.id.tvGoodsClassName, R.color.tv_hint).setBackgroundColor(R.id.catrgory_line, R.color.bg_gray).setDimensionPixelSize(R.id.catrgory_line, R.dimen.tv_line).setImageGrey(R.id.ivGoodsClassImage, url);
    }
