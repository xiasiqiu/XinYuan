...
public class AddSubView extends LinearLayout implements View.OnClickListener, TextWatcher {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        //得到属性
        if (attrs != null) {
            TypedArray typeArray = getContext().obtainStyledAttributes(attrs, R.styleable.AddSubView);
            boolean editable = typeArray.getBoolean(R.styleable.AddSubView_editable, true);
            String location = typeArray.getString(R.styleable.AddSubView_location);
            // 左右两面的宽度
            int ImageWidth = typeArray.getDimensionPixelSize(R.styleable.AddSubView_ImageWidth, -1);
            // 中间内容框的宽度
            int contentWidth = typeArray.getDimensionPixelSize(R.styleable.AddSubView_contentWidth, -1);
            // 中间字体的大小
            int contentTextSize = typeArray.getDimensionPixelSize(R.styleable.AddSubView_contentTextSize, -1);
            // 中间字体的颜色
            int contentTextColor = typeArray.getColor(R.styleable.AddSubView_contentTextColor, 0xff000000);
            // 整个控件的background
            Drawable background = typeArray.getDrawable(R.styleable.AddSubView_all_background);
            // 左面控件的背景
            Drawable leftBackground = typeArray.getDrawable(R.styleable.AddSubView_leftBackground);
            // 右面控件的背景
            Drawable rightBackground = typeArray.getDrawable(R.styleable.AddSubView_rightBackground);
            // 中间控件的背景
            Drawable contentBackground = typeArray.getDrawable(R.styleable.AddSubView_contentBackground);
            // 左面控件的资源
            Drawable leftResources = typeArray.getDrawable(R.styleable.AddSubView_leftResources);
            // 右面控件的资源
            Drawable rightResources = typeArray.getDrawable(R.styleable.AddSubView_rightResources);
            // 资源回收
            typeArray.recycle();

            if ("start".equals(location)) {
                //把布局和当前类形成整体
                LayoutInflater.from(context).inflate(R.layout.view_add_sub_start_layout, this);
            } else if ("end".equals(location)) {
                //把布局和当前类形成整体
                LayoutInflater.from(context).inflate(R.layout.view_add_sub_end_layout, this);
            } else {
                //把布局和当前类形成整体
                LayoutInflater.from(context).inflate(R.layout.view_add_sub_layout, this);
            }
            ...
            if (background != null) {
                ...
                setBackgroundResource(R.drawable.addsubview_add_sub_bg);
            }
    ...
    public void onClick(View view) {
        ...
        if (id == R.id.ic_plus) {
            ...
            if (inputValue < Math.min(mBuyMax, inventory)) {
                inputValue += mStep;
                mOnWarnListener.AddListenter(etInput);
                //正常添加
            ...
            if (inputValue > mBuyMin) {
                inputValue -= mStep;
                mOnWarnListener.ReduceListenter(etInput);
                etInput.setText(inputValue + "");
            } else {
                // 低于最小购买数
                warningForBuyMin();
    ...
    public AddSubView setCurrentNumber(int currentNumber) {
    ...
    public AddSubView setInventory(int inventory) {
        this.inventory = inventory;
        return this;
    }

    public AddSubView setBean(Object bean) {
        this.inventory = inventory;
        return this;
    }
    ...
    public AddSubView setPosition(int position) {
        mPosition = position;
        return this;
    }
    ...
    public AddSubView setStep(int step) {
        mStep = step;
        return this;
    }
    ...
    public interface OnWarnListener {
        ...
        void AddListenter(View num);

        void ReduceListenter(View num);
    }
